...
async function comfyui_execute_and_wait_v2(workflow) {
    ...
    logger.trace('comfyui_execute_and_wait_v2 fixedWorkflow', fixedWorkflow);

    const queueResponse = await comfyui_queue_prompt_v2(fixedWorkflow);
    if (!queueResponse) return { error: true, message: 'No response from ComfyUI', details: 'Queue prompt failed.' };
    processingPrompt = true;
    const promptId = queueResponse.prompt_id;
    await comfyui_track_prompt_progress_v2(promptId);

    const history = await comfyui_get_history_v2(promptId);
    if (!history) return { error: true, message: 'Unknown error', details: 'Please check ComfyUI console.' };

    if (comfyui_isError_v2(history)) {
        const errorMessage = comfyui_getErrorMessage_v2(history);
        return {
            error: true,
    ...
    return { error: false, promptId: promptId, outputs: history[promptId]?.outputs || {} };
}
...
async function comfyui_put_queue_v2(workflow) {
    ...
    const imageData = result.outputs[Object.keys(result.outputs)[0]]?.images?.['0'];
    if (!imageData) return { error: true, message: 'No image output found', details: '' };

    return await comfyui_get_image_v2(imageData);
}
...
async function comfyui_get_image_v2(imageDataToReceive) {
    try {
        ...
        comfyuiLogger.debug(
            'Image data retrieved from server:',
...
async function comfyui_queue_prompt_v2(prompt) {
    try {
        ...
        if (!response.ok) {
            ...
            createToastError(I18nManager.t('toast.httpError'), `status: ${response.status}, message: ${errorText}`);
            return null;
        }
        ...
        const errorTitle = I18nManager.t('toast.text2imageError');
        let errorMessage = '';
        if (error.name === 'TypeError') {
            errorMessage = I18nManager.t('toast.networkError');
        } else if (error.message.includes('HTTP error!')) {
            errorMessage = error.message;
        } else {
            errorMessage = I18nManager.t('toast.checkComfyui');
        }

        console.error('Error details:', error);
        createToastError(errorTitle, errorMessage);
        return null;
    }
}

async function comfyui_get_history_v2(promptId) {
    comfyuiLogger.debug('comfyui_get_history_v2 called. promptId:', promptId);
    try {
