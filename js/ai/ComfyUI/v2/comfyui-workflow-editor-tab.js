...
${comfyuiTypes
    .map((option) => `<option value="${option}" ${option === currentType ? 'selected' : ''}>${option}</option>`)
    .join('')}
</select>
...
<span class="comfui-tab-download" title="Download">Down</span>
...
`;
        ...
        radioBtn.addEventListener('click', async (e) => {
            ...
            document.querySelectorAll(`.comfui-tab-enabled-radio[data-type="${currentType}"]`).forEach((radio) => {
                if (radio !== e.target) {
                    radio.checked = false;
        ...
        button.querySelector('.comfui-tab-save').addEventListener('click', async (e) => {
            e.stopPropagation();
            await this.saveWorkflow();
        });
    ...
    async saveWorkflow() {
        ...
        await comfyUIWorkflowRepository
            .saveWorkflow(this.type || 'T2I', this.id, this.file.name, cleanWorkflow, this.enabled)
            .then(() => {
    ...
    getDisplayableInputCount(nodeId, apiNode) {
        ...
        return Object.entries(apiNode.input.required).filter(([inputName, inputDef]) => {
    ...
    createMissingInput() {
        const missingNode = getText('missingNode');
        const missingDescription = getText('missingDescription');
...
${inputDef[0]
    ...
    .join('')}
        ...
        const type = inputDef[0] === 'INT' || inputDef[0] === 'FLOAT' ? 'number' : 'text';
...
data-input-name="${inputName}">
    ...
    async initializeImagePreviews() {
        const selects = this.contentElement.querySelectorAll('select[data-input-name]');
    ...
    async renderNodes() {
        ...
        const nodes = Object.entries(this.workflow)
            ...
            .map(([id, node]) => {
                ...
                const inputCount = apiNode ? this.getDisplayableInputCount(id, apiNode) : 0;
    ...
    setupInputListeners() {
        this.contentElement
            .querySelectorAll('input[type="text"], input[type="number"], select, textarea')
            .forEach((input) => {
                    ...
                    try {
                        ...
                        if (uploadResult.success) {
                            ...
                            if (select) {
                                ...
                                if (!exists) {
                                    const option = new Option(uploadResult.name, uploadResult.name, true, true);
                                    select.add(option);
                                }
    ...
    async downloadWorkflow() {
        const fixedWorkflow = await comfyui_fixWorkflowTypes_v2(this.workflow);
        const jsonStr = JSON.stringify(fixedWorkflow, null, 2);
