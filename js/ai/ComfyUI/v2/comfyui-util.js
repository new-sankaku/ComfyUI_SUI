...
const comfyuiImageUploadTypes = new Set([
    'I2I',
    'I2I_Inpaint',
    'I2I_Outpaint',
    'I2I_Text',
...
function comfyuiReplacePlaceholders(workflow, requestData, Type = 'T2I') {
    ...
    builder.updateNodesByInputName({
        seed:
            requestData['seed'] === '-1' || requestData['seed'] === '0'
                ? Math.floor(Math.random() * 50000000)
                : requestData['seed'],
        noise_seed:
            requestData['seed'] === '-1' || requestData['seed'] === '0'
                ? Math.floor(Math.random() * 537388471760656)
                : requestData['seed'],
...
function checkWorkflowNodeVsComfyUI(workflowClassTypes) {
    ...
    if (result.length > 0) {
        ErrorGuideDialog.show(ErrorGuideDialog.ERROR_TYPES.WORKFLOW_NODE_MISSING, { missingNodes: result });
        return false;
    } else {
        return true;
    }
