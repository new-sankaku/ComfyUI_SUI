...
function displayGeneratedImage(imageUrl, index, prompt, historyConfig) {
    ...
    downloadBtn.addEventListener('click', function (e) {
        e.stopPropagation();
        downloadImage(item.dataset.imageUrl || imageUrl, index);
    });
    var info = document.createElement('div');
    info.className = 'generated-image-info';
    var timestamp = new Date().toLocaleTimeString('ja-JP');
    info.textContent =
        '#' + (index || container.querySelectorAll('.generated-image-item').length + 1) + ' - ' + timestamp;
...
function clearGeneratedImages() {
    var container = $('generatedImageContainer');
    container.innerHTML =
        '<div id="noImagePlaceholder" style="color:#666;text-align:center;padding:20px;"><div style="font-size:48px;margin-bottom:12px;">🖼️</div><div>' +
        I18nManager.t('status.noGeneratedImages') +
        '</div></div>';
}
