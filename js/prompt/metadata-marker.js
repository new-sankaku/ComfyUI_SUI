async function applyMetadataMarker(imageUrl, prompt) {
    ...
    return new Promise(async (resolve) => {
        ...
        const isWebP =
            ...
            originalData[8] === 0x57 &&
            originalData[9] === 0x45 &&
            originalData[10] === 0x42 &&
            originalData[11] === 0x50;
        const isPNG =
            originalData[0] === 0x89 &&
            originalData[1] === 0x50 &&
            originalData[2] === 0x4e &&
            ...
            canvas.toBlob(
                async (newBlob) => {
                    ...
                    if (originalMetadata && originalMetadata.type === 'webp' && originalMetadata.chunks.length > 0) {
                        const finalData = insertWebPMetadataChunks(
                            new Uint8Array(newArrayBuffer),
                            originalMetadata.chunks
                        );
                        finalBlob = new Blob([finalData], { type: 'image/webp' });
                    } else if (
                        originalMetadata &&
                        originalMetadata.type === 'png' &&
                        originalMetadata.chunks.length > 0
                    ) {
...
function insertPngTextChunks(data, chunks) {
    ...
    chunks.forEach((chunk) => {
        totalChunkSize += chunk.data.length;
    });
...
function insertWebPMetadataChunks(data, chunks) {
    ...
    let totalAddSize = 0;
    chunks.forEach((chunk) => {
        totalAddSize += chunk.data.length;
    });
    ...
    if (hasVP8X) {
        ...
        while (readPos < data.length - 8) {
            ...
            const chunkSize =
                data[readPos + 4] | (data[readPos + 5] << 8) | (data[readPos + 6] << 16) | (data[readPos + 7] << 24);
            const paddedSize = chunkSize + (chunkSize % 2);
