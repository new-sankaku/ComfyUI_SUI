...
function wildcardRefresh() {
    ...
    return wildcardStore
        ...
        .then(function () {
            wildcardList.sort(function (a, b) {
...
function wildcardRender() {
    ...
    var list = wildcardList.filter(function (w) {
        return w.name.toLowerCase().indexOf(q) !== -1;
    });
    if (list.length === 0) {
        c.innerHTML =
            '<div style="padding:20px;text-align:center;color:#666;">' + I18nManager.t('status.noWildcards') + '</div>';
        return;
    }
    var html = '';
...
function wildcardDelete() {
    ...
    wildcardStore
        .removeItem(wildcardEditing.name)
        .then(function () {
            wildcardCloseEditor();
            return wildcardRefresh();
        })
        ...
        for (var i = 0; i < matches.length; i++) {
            (function (match) {
                ...
                if (p.indexOf('*') !== -1) {
                    ...
                    promises.push(
                        wildcardGetItem(p).then(function (w) {
