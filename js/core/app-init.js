...
function updateLoopLineCount() {
    ...
    const positiveLines = positiveText.trim()
        ? positiveText
              .trim()
              .split('\n')
              .filter((p) => p.trim()).length
        : 0;
    const negativeLines = negativeText.trim()
        ? negativeText
              .trim()
              .split('\n')
              .filter((p) => p.trim()).length
...
function updateI2IAngleLineCount() {
    ...
    const angleLines = angleText.trim()
        ? angleText
              .trim()
              .split('\n')
              .filter((p) => p.trim()).length
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    $('btnModeRembgLoop').addEventListener('click', () => switchMode('rembgloop'));
    $('btnModeT2A').addEventListener('click', () => switchMode('t2a'));
    $('btnOpenWorkflow').addEventListener('click', openWorkflowEditor);
    $('btnRandomSeed').addEventListener('click', randomSeed);
    $('btnRandomSeedT2A').addEventListener('click', () => {
        $('t2aSeed').value = Math.floor(Math.random() * 0xffffffff);
    });
    ...
    $('btnGenerateRembgLoop').addEventListener('click', generateImageRembgLoop);
    $('btnGenerateT2A').addEventListener('click', generateImageT2A);
    ...
    $('btnCancelRembgLoop').addEventListener('click', cancelGeneration);
    $('btnCancelT2A').addEventListener('click', cancelGeneration);
    ...
    $('metadataOpacity').addEventListener('input', (e) => {
        $('metadataOpacityValue').textContent = e.target.value + '%';
    });
    $('metadataFont').addEventListener('change', (e) => {
        e.target.style.fontFamily = e.target.value;
    });
    $('metadataFont').style.fontFamily = $('metadataFont').value;
    $('imageModal').addEventListener('click', (e) => {
        if (e.target === $('imageModal') || e.target === $('imageModalInner')) closeImageModal();
    });
    ...
    setupI2IImageUpload(
        ...
        'i2iImagePreview',
        'i2iImageUploadArea',
        () => i2iUploadedFileNames,
        (names) => {
            i2iUploadedFileNames = names;
        },
        'i2iImageCount'
    );
    setupI2IImageUpload(
        'i2iloopImageInput',
        'i2iloopImagePreview',
        'i2iloopImageUploadArea',
        () => i2iloopUploadedFileNames,
        (names) => {
            i2iloopUploadedFileNames = names;
        },
        'i2iloopImageCount'
    );
    setupI2IImageUpload(
        'i2iangleImageInput',
        'i2iangleImagePreview',
        'i2iangleImageUploadArea',
        () => i2iangleUploadedFileNames,
        (names) => {
            i2iangleUploadedFileNames = names;
        },
        'i2iangleImageCount'
    );
    setupI2IImageUpload(
        'upscaleloopImageInput',
        'upscaleloopImagePreview',
        'upscaleloopImageUploadArea',
        () => upscaleloopUploadedFileNames,
        (names) => {
            upscaleloopUploadedFileNames = names;
        },
        'upscaleloopImageCount'
    );
    setupI2IImageUpload(
        'rembgloopImageInput',
        'rembgloopImagePreview',
        'rembgloopImageUploadArea',
        () => rembgloopUploadedFileNames,
        (names) => {
            rembgloopUploadedFileNames = names;
        },
        'rembgloopImageCount'
    );
    ...
    try {
        ...
        setInterval(checkComfyUIConnection, 5000);
        Comfyui_connect();
        await updateWorkflowDisplays();
    } catch (error) {
        createToastError(I18nManager.t('toast.initError'), I18nManager.t('toast.comfyuiConnectionFailed'));
    }
});
