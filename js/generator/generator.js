...
function updateGenerationTimeStats(timeMs) {
    ...
    $('generationTimeStats').textContent = I18nManager.t('status.avgTime')
        .replace('{avg}', avg)
        .replace('{count}', generationTimes.length);
}
function resetGenerationTimeStats() {
    generationTimes = [];
    $('generationTimeStats').textContent = '-';
}
function generateImageNormal() {
    generateImage(parseInt($('normalGenerateCount').value) || 1);
}
async function generateImageLoop() {
    const positivePrompts = $('loopPositivePrompts')
        .value.trim()
        .split('\n')
        .filter((p) => p.trim());
...
function generateImageWildcard() {
    generateImageWildcardExec(parseInt($('wildcardGenerateCount').value) || 1);
}
function generateImageI2I() {
    generateImageI2IExec(parseInt($('i2iGenerateCount').value) || 1);
}
async function generateImageI2ILoop() {
    const positivePrompts = $('i2iloopPositivePrompts')
        .value.trim()
        .split('\n')
        .filter((p) => p.trim());
    ...
    if (i2iloopUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    }
...
async function generateImage(count) {
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            if (isCancelled) {
                $('generationStatus').textContent = I18nManager.t('status.cancelled');
                $('generationStatus').style.color = '#ff9800';
                break;
            }
        ...
        if (!isCancelled) {
            $('generationStatus').textContent = I18nManager.t('status.completed');
            $('generationStatus').style.color = '#4caf50';
        }
    } catch (error) {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('normal');
        setGenerateButtonGenerating('normal', false);
    }
}
async function generateImageLoopExec(positivePrompts, negativePrompts, position, basePositive, baseNegative) {
    ...
    try {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('loop');
        setGenerateButtonGenerating('loop', false);
    }
}
async function generateImageWildcardExec(count) {
    ...
    try {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('wildcard');
        setGenerateButtonGenerating('wildcard', false);
    }
}
async function executeWorkflow(workflow) {
    const result = await comfyui_put_queue_v2(workflow);
    if (!result) throw new Error('No response from ComfyUI');
    if (result.error) {
...
async function handleI2IImageFiles(files, preview, getFileNames, setFileNames, countId) {
    ...
    updateI2IPreview(
        preview,
        successfulUploads.map((r) => r.file),
        newNames,
        getFileNames,
        setFileNames,
        countId
...
async function generateImageI2IExec(countPerImage) {
    if (isGenerating) return;
    if (i2iUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    }
    isGenerating = true;
    ...
    try {
        ...
        $('generationStatus').textContent = I18nManager.t('status.generatingProgress')
            .replace('{current}', 1)
            .replace('{total}', totalGenerations);
        $('generationStatus').style.color = '#ff9800';
        resetGenerationTimeStats();
        ...
        for (let imgIdx = 0; imgIdx < totalImages; imgIdx++) {
            ...
            for (let i = 0; i < countPerImage; i++) {
                ...
                const negativePrompt = await getProcessedNegativePromptForGeneration();
                const currentSeed =
                    baseSeed === -1 ? Math.floor(Math.random() * 0xffffffff) : baseSeed + generationIndex;
                generationIndex++;
                $('generationStatus').textContent = I18nManager.t('status.generatingWithImage')
                    .replace('{current}', generationIndex)
                    .replace('{total}', totalGenerations)
                    .replace('{imgCurrent}', imgIdx + 1)
                    .replace('{imgTotal}', totalImages);
                const requestData = {
                    prompt: prompt,
                    negative_prompt: negativePrompt,
                    seed: currentSeed,
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('i2I');
        setGenerateButtonGenerating('i2I', false);
    }
}
async function generateImageI2ILoopExec(positivePrompts, negativePrompts, position, basePositive, baseNegative) {
    ...
    try {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('i2ILoop');
        setGenerateButtonGenerating('i2ILoop', false);
    }
}
async function generateImageI2IAngle() {
    const anglePrompts = $('i2ianglePrompts')
        .value.trim()
        .split('\n')
        .filter((p) => p.trim());
    if (anglePrompts.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.enterAnglePrompt'));
        return;
    }
    if (i2iangleUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    }
    ...
    try {
        ...
        for (let imgIdx = 0; imgIdx < imageCount; imgIdx++) {
            ...
            for (let i = 0; i < angleCount; i++) {
                ...
                if (result && result.image) {
                    const historyConfig = [{ fieldId: 'i2ianglePrompts', text: $('i2ianglePrompts')?.value?.trim() }];
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('i2IAngle');
        setGenerateButtonGenerating('i2IAngle', false);
    }
}
function generateImageT2A() {
    generateImageT2AExec(parseInt($('t2aGenerateCount').value) || 1);
}
async function generateImageT2AExec(count) {
    ...
    try {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('t2A');
        setGenerateButtonGenerating('t2A', false);
    }
}
async function executeWorkflowAudio(workflow) {
    const result = await comfyui_execute_and_wait_v2(workflow);
    ...
    for (const nodeId in result.outputs) {
        ...
        if (nodeOutput.audio && nodeOutput.audio.length > 0) {
            ...
            const params = new URLSearchParams({
                ...
                subfolder: audioData.subfolder || '',
                type: audioData.type || 'output',
            });
        ...
        if (nodeOutput.images && nodeOutput.images.length > 0) {
            ...
            if (
                imgData.filename &&
                (imgData.filename.endsWith('.mp3') ||
                    imgData.filename.endsWith('.wav') ||
                    imgData.filename.endsWith('.flac'))
            ) {
                const params = new URLSearchParams({
...
function generateImageUpscaleLoop() {
    if (upscaleloopUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    }
...
async function generateImageUpscaleLoopExec() {
    ...
    try {
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('upscaleLoop');
        setGenerateButtonGenerating('upscaleLoop', false);
    }
}
function generateImageRembgLoop() {
    if (rembgloopUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    }
    generateImageRembgLoopExec();
}
async function generateImageRembgLoopExec() {
    if (isGenerating) return;
    if (rembgloopUploadedFileNames.length === 0) {
        createToastError(I18nManager.t('toast.inputError'), I18nManager.t('toast.uploadImage'));
        return;
    ...
    isGenerating = true;
    isCancelled = false;
    showCancelButton('rembgLoop');
    setGenerateButtonGenerating('rembgLoop', true);
    try {
        const baseWorkflow = await comfyUIWorkflowRepository.getEnabledWorkflowByType('REMBG');
        if (!baseWorkflow) {
            ErrorGuideDialog.show(ErrorGuideDialog.ERROR_TYPES.WORKFLOW_NOT_FOUND, { workflowType: 'REMBG' });
            $('generationStatus').textContent = I18nManager.t('status.workflowNotSet');
            $('generationStatus').style.color = '#f44336';
            return;
        }
        const imageCount = rembgloopUploadedFileNames.length;
        $('generationStatus').textContent = I18nManager.t('status.generatingProgress')
            .replace('{current}', 0)
            .replace('{total}', imageCount);
        $('generationStatus').style.color = '#ff9800';
        ...
        for (let imgIdx = 0; imgIdx < imageCount; imgIdx++) {
            ...
            const uploadFileName = rembgloopUploadedFileNames[imgIdx];
            $('generationStatus').textContent = I18nManager.t('status.generatingProgress')
                .replace('{current}', imgIdx + 1)
                .replace('{total}', imageCount);
            const requestData = { uploadFileName: uploadFileName };
            const workflow = comfyuiReplacePlaceholders(baseWorkflow, requestData, 'REMBG');
            generatorLogger.debug('Generated Rembg Loop Workflow JSON:', JSON.stringify(workflow, null, 2));
            const startTime = performance.now();
            const result = await executeWorkflow(workflow);
            const endTime = performance.now();
            const genTime = Math.round(endTime - startTime);
            updateGenerationTimeStats(genTime);
            DashboardUI.recordGeneration('Rembg', genTime, null);
            if (result && result.image) {
                const historyConfig = [];
                displayGeneratedImage(result.image, imgIdx + 1, 'Rembg: ' + uploadFileName, historyConfig);
            }
        }
        if (!isCancelled) {
            $('generationStatus').textContent = I18nManager.t('status.completed');
        ...
        isGenerating = false;
        isCancelled = false;
        hideCancelButton('rembgLoop');
        setGenerateButtonGenerating('rembgLoop', false);
    }
}
